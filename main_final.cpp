...
struct direction_1_2 red_blue(Grid info,int next_hint, struct direction_1_2 information){
    struct node not_pair[1000],extract;
    int top;
...
int pairBFS(char maps[GRID_LENGTH][GRID_LENGTH], int x, int y){
    struct node queueList[1000],cur;
    int head,tail;
    int original;
    ...
    while(head<=tail){
        ...
        if(cur.y+1<GRID_LENGTH){
            if(maps[cur.x][cur.y+1] < 3 && maps[cur.x][cur.y+1]!=( (-1) * ( (10*x) + y ) ) && Does_block_exist(cur.x,cur.y+1)){
            	maps[cur.x][cur.y+1]=( (-1) * ( (10*x) + y ) );
        ...
        if(cur.x+1<GRID_LENGTH){
            if(maps[cur.x+1][cur.y] < 3 && maps[cur.x+1][cur.y]!=( (-1) * ( (10*x) + y ) ) && Does_block_exist(cur.x+1,cur.y)){
            	maps[cur.x+1][cur.y]=( (-1) * ( (10*x) + y ) );
